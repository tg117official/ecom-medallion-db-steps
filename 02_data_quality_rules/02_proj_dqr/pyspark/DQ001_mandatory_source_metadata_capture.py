...
dq_run_id = "DQ001_RUN_001"
rule_name = "Mandatory source metadata capture"
...
validation_columns = ['_ingest_ts', 'bronze_load_date']

extra_validation_columns_by_table = {'bronze_product_catalog_feed': ['_source_file'], 'bronze_customer_review_feed': ['_source_file'], 'bronze_order_event_log': ['_source_file']}

all_dq_metrics = []

print("=" * 80)
print("PROCESSING bronze_brand")
print("=" * 80)

sample_data = [
    ("ID001", "2026-03-10 09:00:00", "2026-03-10"),
    ("ID002", None, "2026-03-10"),
    ("ID003", "2026-03-10 09:20:00", None)
]
schema = "record_id string, _ingest_ts string, bronze_load_date string"
...
# df = spark.table("ecomsphere.bronze.bronze_brand")

table_validation_columns = list(validation_columns)
if "bronze_brand" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_brand"])
...
dq_metrics_df = (
    ...
    .withColumn("table_name", lit("bronze_brand"))
...
# invalid_df.write.format("delta").mode("append").saveAsTable("ecomsphere.silver_quarantine.silver_quarantine.metadata_capture_rejections")

print("=" * 80)
print("PROCESSING bronze_category")
print("=" * 80)
...
# df = spark.table("ecomsphere.bronze.bronze_category")

table_validation_columns = list(validation_columns)
if "bronze_category" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_category"])
table_validation_columns = list(dict.fromkeys(table_validation_columns))
...
dq_metrics_df = (
    ...
    .withColumn("dq_run_id", lit(dq_run_id))
    .withColumn("table_name", lit("bronze_category"))
    .withColumn("rule_name", lit(rule_name))
    .withColumn("rule_category", lit("Schema and Structural Checks"))
    .withColumn("failure_percentage", round((col("failed_records_count") * 100.0) / col("total_records_checked"), 2))
    .withColumn("run_timestamp", current_timestamp())
...
table_validation_columns = list(validation_columns)
if "bronze_warehouse" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_warehouse"])
table_validation_columns = list(dict.fromkeys(table_validation_columns))
...
dq_metrics_df = (
    ...
    .withColumn("dq_run_id", lit(dq_run_id))
    .withColumn("table_name", lit("bronze_warehouse"))
    .withColumn("rule_name", lit(rule_name))
    .withColumn("rule_category", lit("Schema and Structural Checks"))
    .withColumn("failure_percentage", round((col("failed_records_count") * 100.0) / col("total_records_checked"), 2))
    .withColumn("run_timestamp", current_timestamp())
...
table_validation_columns = list(validation_columns)
if "bronze_customer" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_customer"])
table_validation_columns = list(dict.fromkeys(table_validation_columns))
...
table_validation_columns = list(validation_columns)
if "bronze_customer_address" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_customer_address"])
table_validation_columns = list(dict.fromkeys(table_validation_columns))
...
dq_metrics_df = (
    ...
    .withColumn("dq_run_id", lit(dq_run_id))
    .withColumn("table_name", lit("bronze_customer_address"))
    .withColumn("rule_name", lit(rule_name))
    .withColumn("rule_category", lit("Schema and Structural Checks"))
    .withColumn("failure_percentage", round((col("failed_records_count") * 100.0) / col("total_records_checked"), 2))
    .withColumn("run_timestamp", current_timestamp())
...
table_validation_columns = list(validation_columns)
if "bronze_product" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_product"])
table_validation_columns = list(dict.fromkeys(table_validation_columns))
...
table_validation_columns = list(validation_columns)
if "bronze_product_variant" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_product_variant"])
table_validation_columns = list(dict.fromkeys(table_validation_columns))
...
print("=" * 80)
print("PROCESSING bronze_inventory_level")
print("=" * 80)
...
# df = spark.table("ecomsphere.bronze.bronze_inventory_level")

table_validation_columns = list(validation_columns)
if "bronze_inventory_level" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_inventory_level"])
table_validation_columns = list(dict.fromkeys(table_validation_columns))
...
dq_metrics_df = (
    ...
    .withColumn("dq_run_id", lit(dq_run_id))
    .withColumn("table_name", lit("bronze_inventory_level"))
    .withColumn("rule_name", lit(rule_name))
    .withColumn("rule_category", lit("Schema and Structural Checks"))
    .withColumn("failure_percentage", round((col("failed_records_count") * 100.0) / col("total_records_checked"), 2))
    .withColumn("run_timestamp", current_timestamp())
...
table_validation_columns = list(validation_columns)
if "bronze_orders" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_orders"])
table_validation_columns = list(dict.fromkeys(table_validation_columns))
...
table_validation_columns = list(validation_columns)
if "bronze_order_item" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_order_item"])
table_validation_columns = list(dict.fromkeys(table_validation_columns))
...
table_validation_columns = list(validation_columns)
if "bronze_payment" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_payment"])
table_validation_columns = list(dict.fromkeys(table_validation_columns))
...
table_validation_columns = list(validation_columns)
if "bronze_shipment" in extra_validation_columns_by_table:
    table_validation_columns.extend(extra_validation_columns_by_table["bronze_shipment"])
table_validation_columns = list(dict.fromkeys(table_validation_columns))
...
dq_metrics_df = (
    ...
    .withColumn("dq_run_id", lit(dq_run_id))
    .withColumn("table_name", lit("bronze_shipment"))
    .withColumn("rule_name", lit(rule_name))
    .withColumn("rule_category", lit("Schema and Structural Checks"))
    .withColumn("failure_percentage", round((col("failed_records_count") * 100.0) / col("total_records_checked"), 2))
    .withColumn("run_timestamp", current_timestamp())
